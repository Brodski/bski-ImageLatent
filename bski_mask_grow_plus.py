...
#         return io.Schema(
#             node_id="BskiMaskGrowPlus",
#             display_name="Grow Mask Plus",
#             category="mask",
#             description="Grow a batch of masks together while preventing overlaps between masks in the same batch.",
#             search_aliases=["expand masks no overlap", "territory mask grow", "grow masks together"],
#             inputs=[
#                 io.Mask.Input("masks"),
#                 io.Int.Input(
#                     "expand",
...
#     @staticmethod
#     def _normalize_masks(masks) -> torch.Tensor:
#         if isinstance(masks, torch.Tensor):
#             tensor = masks
#         elif isinstance(masks, (list, tuple)):
#             tensor = torch.stack([torch.as_tensor(mask) for mask in masks], dim=0)
#         else:
...
#     @classmethod
#     def execute(cls, masks, expand, tapered_corners) -> io.NodeOutput:
#         mask_batch = cls._normalize_masks(masks)
#         device = mask_batch.device
#         dtype = mask_batch.dtype

#         if mask_batch.shape[0] == 0 or expand == 0:
#             return io.NodeOutput(mask_batch)

#         corner_value = 0 if tapered_corners else 1
#         footprint = np.array(
#             [
#                 [corner_value, 1, corner_value],
#                 [1, 1, 1],
#                 [corner_value, 1, corner_value],
#             ],
#             dtype=bool,
...
#         working = mask_batch.detach().cpu().numpy().copy()
#         epsilon = 1e-6

#         if expand < 0:
#             for _ in range(abs(expand)):
#                 for index in range(working.shape[0]):
#                     working[index] = scipy.ndimage.grey_erosion(
#                         working[index],
#                         footprint=footprint,
#                     )
#         else:
#             for _ in range(expand):
#                 occupied = (working > epsilon).any(axis=0)
#                 dilated = np.empty_like(working)
#                 new_claims = np.zeros_like(working, dtype=bool)

#                 for index in range(working.shape[0]):
#                     dilated[index] = scipy.ndimage.grey_dilation(
#                         working[index],
#                         footprint=footprint,
#                     )
#                     new_claims[index] = (dilated[index] > epsilon) & ~occupied

#                 claim_count = new_claims.sum(axis=0)
#                 # Only uncontested pixels are claimed this step; contested pixels become borders.
#                 claimable = claim_count == 1

#                 if not claimable.any():
#                     break

#                 for index in range(working.shape[0]):
#                     accepted = new_claims[index] & claimable
#                     working[index][accepted] = dilated[index][accepted]

#         result = torch.from_numpy(working).to(device=device, dtype=dtype)
